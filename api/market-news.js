export default async function handler(req, res) {
    ...
    try {
        ...
        const gainersRes = await fetch(`https://brapi.dev/api/quote/list?sortBy=change&sortOrder=desc&limit=3&token=${token}`);
        const gainersData = await gainersRes.json();
        // 2. Buscar Maiores Baixas (Losers)
        // Usamos o endpoint /api/quote/list com sortBy=change e sortOrder=asc
        const losersRes = await fetch(`https://brapi.dev/api/quote/list?sortBy=change&sortOrder=asc&limit=3&token=${token}`);
        const losersData = await losersRes.json();
        // 3. Formatar a resposta
        const response = {
            altas: (gainersData.stocks || []).map((s) => ({
                stock: s.stock,
                name: s.name,
                close: s.close,
                change: s.change,
            ...
            baixas: (losersData.stocks || []).map((s) => ({
                stock: s.stock,
                name: s.name,
                close: s.close,
                change: s.change,
