const getDolarRate = async (token) => {
    try {
...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (isCryptoRequested) {
            ...
            try {
                const cryptoRes = await fetch(`https://brapi.dev/api/v2/crypto?coin=${baseTicker}&currency=BRL&token=${token}`);
            ...
            if (!valor) {
                try {
                    ...
                    const quoteRes = await fetch(`https://brapi.dev/api/quote/${encodeURIComponent(suffix)}?token=${token}`);
        ...
        if (!valor) {
            try {
                const quoteRes = await fetch(`https://brapi.dev/api/quote/${encodeURIComponent(baseTicker)}?token=${token}`);
                const quoteData = await quoteRes.json();
                const result = quoteData.results?.[0];
                if (result && typeof result.regularMarketPrice === 'number') {
                    valor = result.regularMarketPrice;
