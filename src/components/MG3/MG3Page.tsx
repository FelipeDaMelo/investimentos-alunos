...
import { doc, getDoc, setDoc, updateDoc, arrayUnion, deleteDoc, runTransaction, query, collection, orderBy, getDocs, where } from 'firebase/firestore';
...
import { atualizarAtivosMG3 } from '../../utils/atualizarAtivosMG3';
...
export default function MG3Page({
  ...
  useEffect(() => {
    ...
    const fetchMiniRanking = async () => {
      try {
        ...
        const rankingsQuery = query(collection(db, "rankings_mg3"));
        ...
        const usersSnapshot = await getDocs(collection(db, "usuarios_mg3"));
  ...
  const handleAddAtivo = async (novoAtivo: AtivoComSenha, comentario: string) => {
    ...
    try {
      // Pré-carrega o documento do mercado MG3 se for renda variável
      let mg3Doc: any = null;
      if (ativoSemSenha.tipo === 'rendaVariavel') {
        const tickerBuscado = (ativoSemSenha as RendaVariavelAtivo).tickerFormatado.replace('.SA', '').replace('-USD', '').toUpperCase();
        const q = query(collection(db, 'mg3_mercado'), where('ticker', '==', tickerBuscado));
        const snap = await getDocs(q);
        if (!snap.empty) mg3Doc = snap.docs[0];
      }
  ...
  const confirmarVenda = async (quantidadeVendida: number, senhaDigitada: string, comentario: string) => {
    ...
    try {
      let mg3Doc: any = null;
      if (ativoSelecionado.tipo === 'rendaVariavel') {
        const tickerBuscado = (ativoSelecionado as RendaVariavelAtivo).tickerFormatado.replace('.SA', '').replace('-USD', '').toUpperCase();
        const q = query(collection(db, 'mg3_mercado'), where('ticker', '==', tickerBuscado));
        const snap = await getDocs(q);
        if (!snap.empty) mg3Doc = snap.docs[0];
      }
  ...
  return (
    <div className="flex h-screen bg-gray-50 overflow-hidden">
      <Sidebar
        logoOverride="/logo_mostra_cultura.png"
        login={login}
        fotoGrupo={fotoGrupo}
        onLogout={onLogout}
